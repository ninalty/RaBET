...
class WCMapTool(object):
    ...
    def execute(self, parameters, message):
        ...
        if not arcpy.Exists(WCarchiveimage):
            ...
            if len(subdirs[0]) == 39: # collection 1
            ...
            elif len(subdirs[0]) == 40:  # collection 2
                # Parse variables from directory strings # TL
                pathrows = [x[10:16] for x in subdirs]
            ...
            uniquetiles = sorted(set([subdirs[row][4:10] for row in indexintersection])) if len(subdirs[0]) == 39 else sorted(set([subdirs[row][10:16] for row in indexintersection])) # TL [4,10]
            ...
            for tiles in uniquetiles:
                ...
                for imindex, images in enumerate(availableimages):
                    ...
                    img = os.listdir(imagedir + '\\' + images)[0]

                    # Define variables from Landsat directory string
                    currentpathrow = img[10:16]  #TL path/row of current image in loop
                    currentimagename = img[0:25]
                    satellite = img[3:4]
                    pathname = img[11:13]
                    rowname = img[14:16]
                    month = img[21:23]
        ...
        arcpy.management.CopyFeatures(mlradata, tempShapeName)
